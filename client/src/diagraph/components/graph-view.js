...
import {PolicyNodeText, RuleNodeText} from "./node-text";
...
class GraphView extends React.Component {
    ...
    addNewNodes(nodes, oldNodesMap, selectedNode, prevSelectedNode, forceRender = false) {
    ...
    deleteNode(selectedNode) {
        ...
        this.props.onDeleteNode({nodes: this.props.nodes, edges: this.props.edges}, selectedNode, nodeId, newNodesArr);
    }
    ...
    handleEdgeSelected = (e) => {
        ...
        if (source && target) {
            ...
            const originalArrIndex = (this.getEdgeBySourceTarget(source, target)).originalArrIndex;
    ...
    createNewEdge() {
        ...
        if (edgeEndNode) {
            ...
            if (
                ...
                onCreateEdge({nodes: this.props.nodes, edges: this.props.edges}, hoveredNodeData, edgeEndNode);
            } else {
    ...
    handleNodeUpdate = (position, nodeId, shiftKey) => {
        ...
        if (shiftKey) {
            ...
            if (nodeMap) {
                ...
                onUpdateNode({nodes: this.props.nodes, edges: this.props.edges}, nodeMap.node);
            }
        }
        // force a re-render
        this.setState({
...
// Keeps 'zoom' contained
    ...
    dragEdge(draggedEdge) {
        ...
        const off = Edge.calculateOffset(nodeSize, (this.getNodeById(draggedEdge.source)).node, targetPosition, nodeKey);
...
// Renders 'graph' into view element
    ...
    getNodeComponent = (id, node) => {
        const {nodeTypes, nodeSubtypes, nodeSize, renderNode, nodeKey} = this.props;
        let {renderNodeText} = this.props;
        switch (node.type) {
            case 'rule':
                renderNodeText = () => <RuleNodeText data={node} nodeTypes={nodeTypes} isSelected={this.state.selectedNodeObj.node === node}/>;
                break;
            case 'policy':
                renderNodeText = () => <PolicyNodeText data={node} nodeTypes={nodeTypes} isSelected={this.state.selectedNodeObj.node === node}/>;
        }
        return (
            <Node
                key={id}
